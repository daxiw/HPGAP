...
sub MTPHYLOGENY{
	...
	foreach my $temp_ref(keys %{$cfg{mtref}{db}}){
		...
		if(defined $opts{mt_genome_mapping}){
		...
		if(defined $opts{mt_genome_variant_calling}){
			...
			if ( !-d "$outpath/Joint_calling" ) {
				make_path "$outpath/Joint_calling" or die "Failed to create path: $outpath/Joint_calling";
			} 		
			...
			print SH "	-O $outpath/Joint_calling/Joint.HC.vcf.gz && echo \"** Joint.HC.vcf.gz **\"\n";

			print SH "gatk SelectVariants \\\n";
			print SH "	-R $reference \\\n";
			print SH "	-V $outpath/Joint_calling/Joint.HC.vcf.gz \\\n";
			print SH "	--select-type-to-include SNP \\\n";
			print SH "	-O $outpath/Joint_calling/Joint_raw_snps1st.vcf && echo \"** Joint_raw_snps1st.vcf done\" && \\\n";

			print SH "gatk VariantFiltration \\\n";
			print SH "	-R $reference \\\n";
			print SH "	-V $outpath/Joint_calling/Joint_raw_snps1st.vcf \\\n";
			print SH "	--filter-expression \"$cfg{step1}{variant_filtering}{snp}\" \\\n";
			print SH "	--filter-name \"my_snp_filter\" \\\n";
			print SH "	-O $outpath/Joint_calling/Joint_filtered_snps1st.vcf && echo \"** Joint_filtered_snps1st done\" \n";

			print SH "gatk SelectVariants \\\n";
			print SH "	-R $reference \\\n";
			print SH "	-V $outpath/Joint.HC.vcf.gz \\\n";
			print SH "	--select-type-to-include INDEL \\\n";
			print SH "	-O $outpath/Joint_calling/Joint_raw_indels1st.vcf && echo \"** Joint_raw_indels1st.vcf done\" && \\\n";

			print SH "gatk VariantFiltration \\\n";
			print SH "	-R $reference \\\n";
			print SH "	-V $outpath/Joint_calling/Joint_raw_indels1st.vcf \\\n";
			print SH "	--filter-expression \"$cfg{step1}{variant_filtering}{indel}\" \\\n";
			print SH "	--filter-name \"my_indel_filter\" \\\n";
			print SH "	-O $outpath/Joint_calling/Joint_filtered_indels1st.vcf && echo \"** finish mt_genome_joint_calling **\" > $shpath/mt_genome_joint_calling.finished.txt \n";
		...
		if(defined $opts{mt_genome_phylogeny}){
			...
			print SH "seqmagick convert $outpath/Mt_genome_phylogeny/mt_genome.fasta $outpath/Mt_genome_phylogeny/mt_genome.phy\n";
			...
			`perl $Bin/lib/qsub.pl -d $shpath/cmd_mt_genome_phylogeny_qsub -q $cfg{args}{queue} -P $cfg{args}{prj} -l 'vf=4G,num_proc=6 -binding linear:1' -m 100 -r $shpath/cmd_mt_genome_phylogeny.list` unless ($skipsh ==1);
		}
	}	
