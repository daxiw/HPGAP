...
sub SFS{
	...
	foreach my $pop_name (keys %pop){
		...
		if (defined $opts{nonsyn}){
			...
			print IDSH "python $Bin/Tools/easySFS.py -i $slidingwindow_outpath/$pop_name.snpEff.nonsyn.vcf -p $sfs_outpath/$pop_name.list --ploidy $cfg{args}{ploidy} --proj $pop_size -f -a && rm -rf $sfs_outpath/$pop_name/nonsyn_sfs && mv $sfs_outpath/$pop_name/output $sfs_outpath/$pop_name/nonsyn_sfs \n";
			...
			print IDSH "python $Bin/Tools/easySFS.py -i $slidingwindow_outpath/$pop_name.snpEff.syn.vcf -p $sfs_outpath/$pop_name.list --ploidy $cfg{args}{ploidy} --proj $pop_size -f -a && rm -rf $sfs_outpath/$pop_name/syn_sfs  && mv $sfs_outpath/$pop_name/output $sfs_outpath/$pop_name/syn_sfs \n";
		}
