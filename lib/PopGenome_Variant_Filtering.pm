...
sub VariantFiltering {
	...
	open SH, ">$var{shpath}/variant.filtering.s1.sh";
	...
	print SH "vcftools --vcf $var{outpath}/JointCalling/JointCalling_filtered_snps1st.vcf --remove-filtered-all --recode --recode-INFO-all --stdout \| bgzip -c > $var{outpath}/PASS.SNP.vcf.gz && echo \"** finish variant.filtering.s1 **\" > $var{shpath}/variant.filtering.s1.finished.txt \n";
	close SH;

	#switch on the bash running
	open CL, ">$var{shpath}/cmd_variant.filtering.s1.list";
	print CL "sh $var{shpath}/variant.filtering.s1.sh 1>$var{shpath}/variant.filtering.s1.sh.o 2>$var{shpath}/variant.filtering.s1.sh.e\n";
	close CL;
	`perl $Bin/lib/qsub.pl -d $var{shpath}/variant.filtering.s1_qsub -q $cfg{args}{queue} -P $cfg{args}{prj} -l 'vf=1G,num_proc=4 -binding linear:1' -m 100 -r $var{shpath}/cmd_variant.filtering.s1.list` unless (defined $opts{skipsh});
	
	$flag_finish = 0;
	while(1){
		sleep(20);
		my $datestring = localtime();
		print "waiting for variant.filtering.s1 to be done at $datestring\n";
		if(-e "$var{shpath}/variant.filtering.s1.finished.txt"){last;}
	}
	...
	open SH, ">$var{shpath}/variant.filtering.s2.sh";
	print SH "vcftools --gzvcf $var{outpath}/PASS.SNP.vcf.gz $chrcmd --min-meanDP 1 --max-meanDP $avgdp50 --max-missing 0.8 --max-alleles 2 --remove-filtered-all --recode --recode-INFO-all --stdout \| bgzip -c > $var{outpath}/PASS.SNP.DP.vcf.gz && echo \"** finish variant.filtering.s2 **\" > $var{shpath}/variant.filtering.s2.finished.txt\n";
	close SH;
	
	#switch on the bash running
	open CL, ">$var{shpath}/cmd_variant.filtering.s2.list";
	print CL "sh $var{shpath}/variant.filtering.s2.sh 1>$var{shpath}/variant.filtering.s2.sh.o 2>$var{shpath}/variant.filtering.s2.sh.e\n";
	close CL;
	`perl $Bin/lib/qsub.pl -d $var{shpath}/variant.filtering.s2_qsub -q $cfg{args}{queue} -P $cfg{args}{prj} -l 'vf=1G,num_proc=4 -binding linear:1' -m 100 -r $var{shpath}/cmd_variant.filtering.s2.list` unless (defined $opts{skipsh});
	
	$flag_finish = 0;
	while(1){
		sleep(20);
		my $datestring = localtime();
		print "waiting for variant.filtering.s2 to be done at $datestring\n";
		if(-e "$var{shpath}/variant.filtering.s2.finished.txt"){last;}
	}
	...
	open SH, ">$var{shpath}/variant.filtering.s3.sh";
...
EOF
	...
	#switch on the bash running
	`sh $var{shpath}/variant.filtering.s3.sh 1>$var{shpath}/variant.filtering.s3.sh.o 2>$var{shpath}/variant.filtering.s3.sh.e` unless (defined $opts{skipsh});
